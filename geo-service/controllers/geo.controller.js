...
export const reverseGeocode = async (req, res) => {
  try {
    ...
    if (!latitude || !longitude) {
      return res
        .status(400)
        .json({ error: 'Параметры "latitude" и "longitude" обязательны' });
    }
...
export const getCityByCoordinates = async (req, res) => {
  try {
    ...
    const location = await googleMapsService.getCityByCoordinates(
      latitude,
      longitude
    );
...
export const getDistanceAndDuration = async (req, res) => {
  try {
    ...
    if (!origin || !destination) {
      return res
        .status(400)
        .json({ error: 'Параметры "origin" и "destination" обязательны' });
    }
  ...
  try {
    ...
    if (!driverId || !latitude || !longitude) {
      return res.status(400).json({
        error: 'Параметры "driverId", "latitude" и "longitude" обязательны',
      });
    }
...
export const getApiUsageStats = async () => {
  const stats = await redis.hgetall("apiRequestCounts");
  const deliveryOrders = await order.count({ where: { type: "delivery" } });
  return { ...stats, deliveryOrders };
};
